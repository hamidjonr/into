...
const Exchange = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(defaultProducts, (snapshot) => {
            const exchList = snapshot.docs.map((doc) => ({
                id: doc.id,
                ...doc.data()
            }));
            setExchangeItems(exchList);
        }, (err) => {
            console.error('Error fetching exchange items:', err);
        });
            ...
            <section className="text-gray-600 body-font">
                <div className="container px-5 py-4 mx-auto">
                    <div className="flex flex-wrap -m-4">
                        {exchangeItems.filter(item => exType === item.type).map((item) => (
